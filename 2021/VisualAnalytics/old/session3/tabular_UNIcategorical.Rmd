...
When dealing with tabular data, you can suspect that you can produce a visualization for each column, and then for a couple of them, and then for three or more. 

Let's pay attention to the _univariate_ case; which is common for searching problems or verifying outcomes; not for giving explanations. Then, when dealing with univariate data, you need to be aware of two things: _what question you are trying to answer_; and _how to treat a particular variable_ to build the plot that will answer that question.
...
If you prefer the range from 0 to 100:
```{r, eval=TRUE}
...
The **missing** label can be renamed:
```{r, eval=TRUE}
...
* Add the reference lines: Use ti to show a particular threshold.
...
base = ggplot(tableFreq, aes(x=locale,pct)) + theme_classic()
base = base + scale_x_discrete(limits=localesOrd) 
...
```{r, eval=TRUE}
# would you add: legend.background = element_rect(fill='gray90')?
...
Box plots have important statistical information. The beginning and the ending of the box indicates the first (q1) and the third quantile (q75); and the thicker line in the middle represents the median. All those values are clearly visible, but we can not retrieve the values using **summary()**:

```{r, eval=TRUE}
summary(eduwa$High.Grade)
```

When a variable is ordinal, that function just gives you the counts. However, you can try:
```{r, eval=TRUE}
summary(as.numeric(eduwa$High.Grade))
```

Those values now represent positions, then:
```{r, eval=TRUE}
positions= as.integer(summary(as.numeric(eduwa$High.Grade)))
positions
```

Which you can reuse like this:

```{r, eval=TRUE}
levels(eduwa$High.Grade)[c(positions[2],positions[3],positions[5])]
```

This way you can recover those values.

An alternative way can be using **ggplot_build**:
...
```{r advancedTable, warning=FALSE,results='asis', eval=TRUE,message=FALSE}
...
Improve  **_lolliplot7_** by adding or erasing elements.


### Exercise 4:

Improve  **_box2_**, making sure that only the quartiles are shown on the y axis.
...
Prepare a plot for a numeric variable from your data base, by first **cutting** as shown above. 
